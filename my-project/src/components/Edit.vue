 	...
 	<h1 class="page-header">编辑用户</h1>
	 	<form v-on:submit="updateCustomers">
	 		<div class="well">
				...
				<div class="form-group">
	 			<label>毕业学校</label>
	 			<input type="text" class="form-control" placeholder="graduateschool" v-model="customer.graduateschool">
	 			</div> 			
				<div class="form-group">
	 			<label>职业</label>
  		...
  		if(!this.customer.name || !this.customer.phone || !this.customer.email){
			this.alert = '请添加对应的信息'		
  			...
  			let updateCustomer = {
  				...
  				graduateschool : this.customer.graduateschool,
  				profession : this.customer.profession,
  				profile : this.customer.profile
  			}
  			this.axios.put('http://localhost:3000/users/'+this.$route.params.id,updateCustomer)
  			.then((response)=>this.$router.push({path:'/',query:{alert:'用户信息更新成功'}}))
  		}
  		e.preventDefault()
  	}
  },
